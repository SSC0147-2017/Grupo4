...
func _ready():
	var file = File.new()
	file.open("res://Text//Aqui.txt", file.READ)
	file.seek(position)
	...
	while(!str(nextLine[0]).match("end_config")):
		if str(nextLine[0]).match("left"):
			...
			if str(nextLine[0]).match("00"):
				get_tree().get_root().get_node("Background/char1").set_texture(load("res://Source//Cutscenes//00.png"))
		...
		if str(nextLine[0]).match("right"):
			...
			if str(nextLine[0]).match("00"):
				get_tree().get_root().get_node("Background/char2").set_texture(load("res://Source//Cutscenes//00.png"))
			if str(nextLine[0]).match("01"):
